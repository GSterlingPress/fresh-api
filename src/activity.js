...
function isCoreEvent(e){return e.route==='POST /v1/check'||e.route==='POST /v1/observe'||e.route==='tools/call:fresh_check'||e.route==='tools/call:fresh_observe'}
function isRealWorldEvidence(e){return e.classification==='UNKNOWN_MACHINE'||e.classification==='CREDIBLE_REAL_USE'}
export class ActivityStore{
 ...
 snapshot(){const credible=this.events.filter(e=>e.classification==='CREDIBLE_REAL_USE');const unique=new Set(credible.map(e=>e.caller));const realWorldCore=this.events.filter(e=>isCoreEvent(e)&&isRealWorldEvidence(e));const realWorldCallers=new Set(realWorldCore.map(e=>e.caller));const milestones=[];for(const e of credible){if(milestones.some(x=>x.caller===e.caller))continue;milestones.push({number:milestones.length+1,achieved:true,at:e.at,caller:e.caller,classification:e.classification,reasons:e.classificationReasons});if(milestones.length===10)break}while(milestones.length<10)milestones.push({number:milestones.length+1,achieved:false});return {service:'FRESH',verificationPolicy:VERIFICATION_POLICY,realUse:{allTimeRealWorldCalls:realWorldCore.length,allTimeRealWorldCallers:realWorldCallers.size,verifiedStrangers:unique.size},realWorldConversionMilestones:milestones,privacy:{rawIpStored:false,requestPayloadStored:false,fullTargetUrlStored:false},feed:this.events.slice(0,500)}}
}
